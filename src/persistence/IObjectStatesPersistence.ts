...
export interface IObjectStatesPersistence extends IGetter<ObjectStatesV1, string>, IWriter<ObjectStatesV1, string> {
    ...
    addOne(correlationId: string, orgId: string, startTime: Date, endTime: Date, state: ObjectStateV1,
        callback: (err: any, item: ObjectStatesV1) => void): void;

    addBatch(correlationId: string, states: ObjectStatesV1[],
        callback: (err: any) => void): void;
            
    update(correlationId: string, item: ObjectStatesV1,
        callback: (err: any, item: ObjectStatesV1) => void): void;
