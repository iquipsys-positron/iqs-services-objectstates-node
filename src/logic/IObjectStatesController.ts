...
export interface IObjectStatesController {
    ...
    getTimelineStates(correlationId: string, time: Date, filter: FilterParams,
        callback: (err: any, states: ObjectStateV1[]) => void): void;

    addState(correlationId: string, state: ObjectStateV1, 
        callback?: (err: any) => void): void;

    addStates(correlationId: string, states: ObjectStateV1[], 
        callback?: (err: any) => void): void;
            
    deleteStates(correlationId: string, filter: FilterParams,
        callback?: (err: any) => void): void;
}
